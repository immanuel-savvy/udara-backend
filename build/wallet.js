...
var brass_callback = function brass_callback(req, res) {
  var event = req.body; // validate event

  var hash = require("crypto").createHmac("sha256", brass_webhook_secret).update(JSON.stringify(event)).digest("hex"); // return appropriate response if the request hash does not match the header


  if (hash != req.headers["X-Brass-Signature"]) return res.status(401).json({
    message: "Unauthorized request."
  }); // do something with event

  _ds_conn.LOGS.write(event);
